...
\begin{tikzpicture}
   ...
   \broker{b0}{\segmentSize}{below = 0.1cm of ALIGNRECTANGLE.north west, anchor=north}{ 
       {  
  ...
  \node[above = 0.15cm of ALIGNRECTANGLE.100, anchor=center] (partition_label) {partition};
  ...
  \node[below = 0.4cm of b0_par0_r6.south, anchor=north] (new_rec_label) {new record};
  \draw [->] (new_rec_label) edge (b0_par0_r6);
  
  \node[above = 0cm of ALIGNRECTANGLE.south] () {\normalsize Kafka Broker};
  \node[above = 0cm of INNERRECTANGLE.south, anchor=south] () {\normalsize Kafka Node};
  ...
  \node[above left = 0.3cm and 0.2 cm of ALIGNRECTANGLE.west, anchor=east] (topic_label) {Topic};
  \draw [to path={|- (\tikztotarget)}]
