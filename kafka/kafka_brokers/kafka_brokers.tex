...
\newcommand{\broker}[4] {% {Id}{Size}{Location}{partitions}
  ...
  \node[#3] (\brokerId) {};
  \pgfmathtruncatemacro\pos{0} 
  
  \foreach \id/\r/\c in #4 {
    \partition{\brokerId_\id}{\recordSize}{below = \pos cm of \brokerId, anchor=north}{\r}{\c}  
    \pgfmathparse{(\pos + (2.00 * \recordSize) }
...
\begin{document}
  ...
  \pgfmathsetmacro\fontSize{\diagramSize * 4}
  ...
  \pgfmathsetmacro\frameRectangleWidth{\outlineSize  * 1.15}
...
\begin{tikzpicture}
   ...
   \draw [-, dotted] (c0)  edge  (LEFTINNERRECTANGLE.140);
   \draw [-, dotted] (c0)  edge  (LEFTINNERRECTANGLE.220);
   ...
   \broker{b0}{\segmentSize}{below = 0.1cm of LEFTALIGNRECTANGLE.north west, anchor=north}{ 
       {  
   ...
   \tikzstyle{topicLeaderColor} = [fill=red!50]
   \tikzstyle{topicFollowerColor} = [fill=red!10]
   \broker{b1}{\segmentSize}{below = 0.1cm of RIGHTALIGNRECTANGLE.north west, anchor=north}{ 
       {  
  ...
  \node[above = 0.15cm of LEFTALIGNRECTANGLE.100, anchor=center] (partition_label) {partition};
  \draw [to path={-| (\tikztotarget)}]
    (partition_label) edge (b0_par0_r1.120)
    (partition_label) edge (b0_par0_r5)
  ;
  
  \node[above = 0.10cm of RIGHTALIGNRECTANGLE.100, anchor=center] (partition_label2) {partition replica};
  \draw [to path={-| (\tikztotarget)}]
    (partition_label2) edge (b1_par0_r1.120)
    (partition_label2) edge (b1_par0_r5)
  ;
  
  \node[above = 0.05cm of b0_par0_r2, anchor=south, inner sep=0mm] (seg_label) {segment};
  \draw [to path={-| (\tikztotarget)}]
    (seg_label) edge (b0_par0_r1)
    (seg_label) edge (b0_par0_r3)
  ;
  ...
  \node[below = 0.2cm of b0_par0_r4, anchor=north] (rec_label) {record};
  \draw [->] (rec_label) edge (b0_par0_r4);
  
  \node[below = 0.3cm of b0_par0_r6.south, anchor=north] (new_rec_label) {new record};
  \draw [->] (new_rec_label) edge (b0_par0_r6);
  
  \node[above = 0cm of LEFTALIGNRECTANGLE.south] () {\normalsize Kafka Broker 0 };
  \node[above = 0cm of LEFTINNERRECTANGLE.south, anchor=south] () {\normalsize Kafka Node 0};
 
  \node[above = 0cm of RIGHTALIGNRECTANGLE.south] () {\normalsize Kafka Broker 1};
  \node[above = 0cm of RIGHTINNERRECTANGLE.south, anchor=south] () {\normalsize Kafka Node 1};
  ...
  \node[l_leader, right=0.2cm of RIGHTINNERRECTANGLE.318,
  label={[label distance=0cm]0:{\fontsize{\fontSize pt}{\fontSep pt}\selectfont Leader}}
       ] (l_leader) {};
  \node[l_follower, below=0cm of l_leader,
  label={[label distance=0cm]0:{\fontsize{\fontSize pt}{\fontSep pt}\selectfont Follower}}
  ] (l_follower) {};
  
  
  \node (topic_label) at ($(LEFTINNERRECTANGLE)!0.5!(RIGHTINNERRECTANGLE)$) {Topic};
  \draw [to path={|- (\tikztotarget)}]
   ...
   (topic_label.145) edge (b0_par1_r2)
   (topic_label.45) edge (b1_par1_r1)
  ;
  
  \pgfmathsetmacro\lSize{\segmentSize * 0.5}
  \loggy{l0}{\lSize}{above=0cm of b0_par0_r6.center, anchor=center}{180}{}
 
  \draw[bend right=15, ->] (b0_par0_r6) edge (b1_par0_r6.center);
