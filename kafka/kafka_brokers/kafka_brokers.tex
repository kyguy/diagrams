...
\begin{tikzpicture}
  ...
  \node[above = 0.15cm of LEFTALIGNRECTANGLE.100, anchor=center] (partition_label) {\bfseries{partition}};
  \draw [to path={-| (\tikztotarget)}]
    (partition_label) edge (b0_par0_r1.north west)
    (partition_label) edge (b0_par0_r5.north east)
  ;
  
  \node[above = 0.10cm of RIGHTALIGNRECTANGLE.100, anchor=center] (partition_label2) {\bfseries{partition replica}};
  \draw [to path={-| (\tikztotarget)}]
    (partition_label2) edge (b1_par0_r1.north west)
    (partition_label2) edge (b1_par0_r5.north east)
  ;
  
  \node[above = 0.05cm of b0_par0_r2, anchor=south, inner sep=0mm] (seg_label) {\bfseries{segment}};
  \draw [to path={-| (\tikztotarget)}]
    (seg_label) edge (b0_par0_r1.130)
    (seg_label) edge (b0_par0_r3.north east)
  ;
  ...
  \node[below = 0.35cm of b0_par0_r6.south, anchor=north] (new_rec_label) {\bfseries{new record}};
