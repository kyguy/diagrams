...
\begin{document}
  ...
  \pgfmathsetmacro\alignRectangleHeight{\outlineSize * 0.5}
...
\begin{tikzpicture}
   ...
   \connector{con2}{0.7}{above = 0.3 cm of INNERRECTANGLE.230, anchor=south}{\fluentdStandalone}{ {} }
   \node[above = 0cm of con2.south, anchor=north, outer sep = -0.5mm] () {Connector};
   
  \node[align=center, below = 0.7cm of INNERRECTANGLE.100] (ct) {Connector Activation};
  \node[ below = 0. cm of INNERRECTANGLE.125, anchor=north,
          minimum height=0.1 cm,
          inner sep=0mm, 
          label={[align=center]center:REST\\INTERFACE}
        ] (API) {
    \begin{tikzpicture}
      \fill[thick, gray, opacity=0.5] \gear{8}{0.6}{0.5}{10}{2}{0.3};
    \end{tikzpicture}}; 
   ...
   \node[align=center] (ct) at ($(con0)!0.5!(con1)$) {Connector\\Tasks};
   \draw [->] (ct)  edge  (con0.south);
   \draw [->] (ct)  edge  (con1.north);

  \node[left = 0.20 cm of FRAMECIRCLE.center](){\normalsize Worker};
  \node[above = 0 cm of INNERRECTANGLE.south](){\normalsize Kafka Connect Node};
  
  \node[align=center, above = 0 cm of ALIGNRECTANGLE.north] (ed_label) {External data};
  \draw [->, to path={|- (\tikztotarget)}]
    (ed_label)  edge  (par0_1)
    (ed_label)  edge  (par2_1)
    ;
  ...
  % DATA FLOW
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \node[above = 0cm of ALIGNRECTANGLE.161] () {External Data Flow};
